...
<div class="w-full px-0 space-y-6">
                ...
                <!-- Introduction -->
                <div class="bg-blue-400/20 border border-blue-400/30 rounded-lg p-4 backdrop-blur-sm">
                    <div class="flex items-center space-x-3">
                        <svg class="w-5 h-5 text-blue-400" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path>
                        </svg>
                        <div>
                            <h3 class="text-sm font-medium text-blue-400">Overcharge Protection</h3>
                            <p class="text-sm text-blue-200">Charge renters for late returns or out-of-city use to protect your vehicles</p>
                        </div>
                    </div>
                </div>

                <!-- Quick Stats -->
                <div class="glass-card-dark border border-white/20 rounded-lg p-6 backdrop-blur-sm" v-if="stats">
                    <h3 class="text-lg font-medium text-white mb-4">Your Overcharge Summary</h3>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="text-center p-3 bg-green-400/20 border border-green-400/30 rounded-lg backdrop-blur-sm">
                            <div class="text-2xl font-bold text-green-400">₱{{ formatCurrency(stats.total_overcharges || 0) }}</div>
                            <div class="text-xs text-white/60">Total Earned</div>
                        </div>
                        <div class="text-center p-3 bg-yellow-400/20 border border-yellow-400/30 rounded-lg backdrop-blur-sm">
                            <div class="text-2xl font-bold text-yellow-400">₱{{ formatCurrency(stats.unpaid_overcharges || 0) }}</div>
                            <div class="text-xs text-white/60">Unpaid</div>
                        </div>
                        <div class="text-center p-3 bg-red-400/20 border border-red-400/30 rounded-lg backdrop-blur-sm">
                            <div class="text-2xl font-bold text-red-400">{{ stats.late_returns_count || 0 }}</div>
                            <div class="text-xs text-white/60">Late Returns</div>
                        </div>
                        <div class="text-center p-3 bg-blue-400/20 border border-blue-400/30 rounded-lg backdrop-blur-sm">
                            <div class="text-2xl font-bold text-blue-400">{{ stats.out_of_city_count || 0 }}</div>
                            <div class="text-xs text-white/60">Out of City</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card-dark border border-white/20 rounded-lg p-6 backdrop-blur-sm">
                    <form @submit.prevent="updateSettings" class="space-y-6">
                        ...
                        <div class="flex items-center justify-between pb-4 border-b border-white/20">
                            <div>
                                <h3 class="text-lg font-medium text-white">Enable Overcharges</h3>
                                <p class="text-sm text-white/70">Turn on automatic overcharge calculations for all your vehicles</p>
                            </div>
                        ...
                        <div v-if="form.enable_overcharges" class="space-y-8">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    class="block w-full bg-white/10 border-white/20 text-white placeholder-white/50 rounded-md shadow-sm focus:border-blue-400 focus:ring-blue-400/50 backdrop-blur-sm"
                                    placeholder="30"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input
                                        ...
                                        class="block w-full pl-8 bg-white/10 border-white/20 text-white placeholder-white/50 rounded-md shadow-sm focus:border-blue-400 focus:ring-blue-400/50 backdrop-blur-sm"
                                        placeholder="100.00"
                                    />
                            ...
                            <div class="bg-white/5 border border-white/20 rounded-lg p-4 backdrop-blur-sm">
                                <h4 class="text-sm font-medium text-white mb-2">Examples</h4>
                        ...
                        <!-- Save Button -->
                        <div class="flex items-center justify-end pt-6 border-t border-white/20">
                            <button
                                type="submit"
                                :disabled="form.processing"
                                class="bg-blue-400 hover:bg-blue-500 text-white font-medium py-2 px-4 rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors backdrop-blur-sm"
                            >
